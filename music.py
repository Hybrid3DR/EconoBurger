...
class Music(commands.Cog):
    ...
    async def skip(self, ctx):
        ...
        poll = discord.Embed(title=f"Vote to Skip Song by - {ctx.author.name}#{ctx.author.discriminator}", description="**75% of the voice channel must vote to skip for it to pass.**", colour=discord.Colour.blue())
        ...
        if not skip:
            embed = discord.Embed(title="Skip Failed", description="*Voting to skip the current song has failed.*\n\n**Voting failed, the vote requires at least 75% of the members to skip.**", colour=discord.Colour.red())
    ...
    async def forceskip(self,ctx):
        voice.pause()
        play_next(ctx)
        await ctx.send('Force skip executed!')
